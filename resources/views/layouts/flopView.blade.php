...
<body class="bg-violet-200 w-full pt-5">
    ...
    <div class='w-full bg-violet-300 flex flex-col justify-around'>
        <div class="bg-red-800 w-7/12 self-center flex flex-col content-center rounded-lg p-4">
            ...
            <div class="mb-6">
            <input type="number" id="cPCode" min='1000' max='9999' placeholder="Postal Code of the City" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            </div>
            <button id='addcDetail' class="mb-8 bg-amber-400 text-sm rounded-lg">Add Data</button>
        </div>
        <div class="bg-amber-200 text-center text">
        <?php

            foreach($data as $key => $val ){

                echo $val->Uniqueid.'<br/>';
                echo $val->CityName.'<br/>';
                echo $val->CityMayor.'<br/>';
